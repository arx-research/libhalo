...
export type TSignDigestCallback = {
  (digest: HexString): Promise<HaloResSign>;
}
...
export function createViemHaloAccount(address: Hex, signDigestCallback: TSignDigestCallback): LocalAccount {
  return toAccount({
    address: getAddress(address),

    async signMessage({ message }: { message: SignableMessage }): Promise<Hex> {
      const digest = hashMessage(message)
      const haloResSign = await signDigestCallback(digest)
